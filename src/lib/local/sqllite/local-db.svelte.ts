...
let worker: Worker;
export let workerInitialized = Promise.withResolvers<void>();
export let dataInitialized = Promise.withResolvers<void>();
...
export async function initWorker() {
	...
	sendMessage<string[]>({ type: WorkerMessageType.DB_INIT }).then((result) => {
		if (result.length > 0) initializeTables(result).then(() => dataInitialized.resolve());
		else dataInitialized.resolve();
	});
...
async function sendMessage<T>(message: Omit<WorkerMessage, "id">) {
	return new Promise<T>(async (resolve) => {
		...
		if (message.type !== WorkerMessageType.DB_INIT) await dataInitialized.promise;
