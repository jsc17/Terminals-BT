...
<div class="search-results card">
	<p class="center error">Links are updated (9/2 - Fixed a few issues, so they should hopefully all work now). Data updates will follow soon.</p>
						...
						{#snippet renderItem(item)}
							<div class={{ "virtual-list-row": !appWindow.isMobile, "virtual-list-row-mobile": appWindow.isMobile }}>
								{#if item}
									...
									<div class="inline">
										<a
											class={{ "unit-name": true }}
											href="http://masterunitlist.battletech.com/units/{item.name
												.normalize('NFD')
												.replaceAll(/[^\w\s\-]/g, '')
												.replaceAll(' ', '-')}"
											target="_blank"
