...
export const getUsersLists = query(async () => {
	...
	const results = await prisma.listV3.findMany({ where: { userId: locals.user.id }, select: { id: true, name: true, rules: true } });

	return { status: "success", data: results };
});

export const loadUserList = query(v.string(), async (listId) => {
	const listData = await prisma.listV3.findUnique({ where: { id: listId } });
