...
export class ListProps {
	...
	get haveScrollPropsChanged() {
		return this.previous_state.scrollToIndex !== this.scrollToIndex || this.previous_state.scrollToAlignment !== this.scrollToAlignment;
	}

	get haveSizesChanged() {
		return this.previous_state.itemCount !== this.itemCount || this.previous_state.itemSize !== this.itemSize || this.previous_state.estimatedItemSize !== this.estimatedItemSize;
	}
	...
	get haveDimsOrStickyIndicesChanged() {
		return (
			this.previous_state.height !== this.height ||
	...
	listen(scrollToIndex, scrollToAlignment, scrollOffset, itemCount, itemSize, estimatedItemSize, height, width, stickyIndices) {
		...
		if (typeof itemSize === "number" || typeof itemSize === "function" || Array.isArray(itemSize)) this.itemSize = itemSize;

		if (typeof estimatedItemSize === "number") this.estimatedItemSize = estimatedItemSize || this.itemSize || 50;

		if (typeof height === "number" || typeof height === "string") this.height = height;

		if (typeof width === "number" || typeof width === "string") this.width = width;
