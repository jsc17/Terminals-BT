import type { FormationV2, List, UnitV2 } from "$lib/types";
import { getFormationDataFromName } from ".";
...
function checkSize(units: UnitV2[], size: number, limit: "equal" | "equalOrGreater" | "equalOrLess", amount: number) {
	let unitsOfSize = 0;
	switch (limit) {
		case "equal":
...
function checkRoles(units: UnitV2[], roles: string[], amount: number, alternateRoles?: string[], alternateAmount?: number) {
...
function checkArmor(units: UnitV2[], armor: number, amount: number) {
	return units.length != 0 && units.filter((unit) => (unit.baseUnit.armor ?? 0) > armor).length >= amount ? 1 : -1;
}
function checkDamage(units: UnitV2[], damage: number, range: string, limit: "equal" | "equalOrGreater" | "equalOrLess", amount: number) {
...
function checkMovement(units: UnitV2[], speed: number, amount: number, jumpException?: number) {
	return units.length != 0 &&
		units.filter((unit) => {
...
function checkAbility(units: UnitV2[], abilities: string[], amount: number) {
	return units.length != 0 &&
		units.filter((unit) => {
...
function checkAerospacePair(units: UnitV2[]) {
	if (units.length != 2) {
		return -1;
	}
	return units[0].baseUnit.name == units[1].baseUnit.name ? 1 : -1;
}
function checkTransport(units: UnitV2[], secondaryUnits: UnitV2[]) {
