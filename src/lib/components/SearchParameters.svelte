  ...
  <div class="card" class:hidden={appWindow.isMobile && !showParameters}>
    <div class={appWindow.isMobile ? "parameters-mobile" : "parameters"}>
      <div class="parameter">
        ...
        <Combobox
          items={allowedEras}
          bind:value={resultList.selectedEras}
          inputProps={{ clearOnDeselect: true, placeholder: "Any" }}
          type="multiple"
        ></Combobox>
      </div>
      <div class="selected-container">
        ...
        {#each resultList.selectedEras.slice(0, 4) as selected}
          <button
            ...
            onclick={() => {
              resultList.selectedEras = resultList.selectedEras.filter(
                (text) => {
      ...
      <div class="parameter">
        <p>Faction:</p>
        <Combobox
          items={allowedFactions}
          bind:value={resultList.selectedFactions}
          inputProps={{ clearOnDeselect: true, placeholder: "Any" }}
          type="multiple"
        ></Combobox>
      </div>
      <div class="selected-container">
        ...
        {#each resultList.selectedFactions.slice(0, 4) as selected}
          <button
            class="selected-block"
            onclick={() => {
              resultList.selectedFactions = resultList.selectedFactions.filter(
                (text) => {
                  return text != selected;
                }
              );
      ...
      <div class="inline">
        <input
          ...
          disabled={resultList.selectedEras.length != 1 ||
            resultList.selectedFactions.length != 1}
        />
      ...
      <div class="general-notice">
        You can select additional general lists from the faction dropdown
      </div>
      <button
        id="getData"
        onclick={() => {
    ...
    {#if resultList.selectedEras.length != resultList.eras.length || !resultList.eras.every( (era) => resultList.selectedEras.includes(era.toString()) ) || resultList.selectedFactions.length != resultList.factions.length || !resultList.factions.every( (faction) => resultList.selectedFactions.includes(faction.toString()) )}
      <div class="center">
        ...
        <p class="general-notice">
          Faction or Era selection changed. Press search to get updated results
        </p>
      </div>
    {/if}
  </div>
