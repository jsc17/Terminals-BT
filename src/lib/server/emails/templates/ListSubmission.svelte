  ...
  <Section>
    ...
    {#if parsedFixedUnits.length > 0 || parsedAddedUnits.length > 0 || parsedFixedBfs.length > 0 || parsedAddedBfs.length > 0}
      ...
      <Text style={fixedStyle}
        >Units or BFS were manually added or edited. Please Review the list and the attached PDF carefully before approval. The list was successfully validated
        while including these changes.</Text
      >
    {/if}
